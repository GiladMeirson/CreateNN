...
const RenderGenerate=()=>{
    ...
    <p class="textP">what did you draw ?</p>
    <input type="text" name="labelIN" id="labelIN">
    <button onclick="SubmitPack()" class="button" >SUBMIT</button>
    <button onclick="downloadData()" class="button" id="Download" >Download Data</button>
    `;
...
const testThis=()=>{
    ...
    const predictFunc = new Function('input',model);
    const result = predictFunc(pack.input);
    console.log(result);
    const res = findMaxValueAndIndex(result);
    $('#predictP').html(`you draw: ${res.index},<br> im sure at ${(res.value*100).toFixed(2)}%`)
...
const GenerateThis=()=>{
    ...
    let packLocal ={
        ...
        output: new Array(256).fill(0)
    }
    let index = parseInt($("#inputIN").val());
    packLocal.input[index] = 1;
    const predictFunc = new Function('input',GENmodel);
    const result = predictFunc(packLocal.input);
    packLocal.output = result;
    console.log(result);
    result.forEach((val,index)=>{
        if (val>_NoiseFilter){
            $(`#td${index+1}`).css("background-color", "black");
        }
    })
}

const findMaxValueAndIndex = (array) => {
...
const GenerateMode=()=>{
    ...
    <br><br>
    <button onclick="GenerateThis()" class="button">Draw</button>
    <label for="inputIN">input number between 0-9 </label>
    <input type="number" name="inputIN" id="inputIN">
    <br><br>
    <label id="noiseMuteLabel" for="noiseMuteIN">cancel the noises : 0.5</label>
    <input onchange="changeNoise(this)" name="noiseMuteIN" id="noiseMuteIN" type="range" step="0.01" min="0.01" max="0.99" value="0.5" class="slider2">
...
const changeNoise=(elm)=>{
    _NoiseFilter = parseFloat(elm.value);
    $('#noiseMuteLabel').html(`cancel the noises : ${elm.value}`);
}